...
class DatatablesPLSQL
{
    ...
    public function getResults($request, $table, $filters, $joins, $defaultWhere, $defaultGroup, $debug)
    {
        ...
        foreach($filters as $filterKey => $filterValue)
        {
            ...
            {
                if($selectStatement != "")
                    $selectStatement = $selectStatement.", TO_CHAR(".$filterKey.", 'DD/MON/YYYY') AS ".$filterValue["alias"];
                else
            ...
            {
                if($selectStatement != "")
                    $selectStatement = $selectStatement.", TO_CHAR(".$filterKey.", 'DD/MON/YYYY HH24:MI:SS') AS ".$filterValue["alias"];
                else
                    $selectStatement = $filterKey." AS '".$filterValue["alias"]."'";
            }
            else
        ...
        foreach($filters as $filterKey => $filterValue)
        {
            if($filterValue["value"] != "")
            {
                ...
                if($method == "LIKE"){
                    ...
                    if($date != null){
                        $whereStatement .= ($whereStatement == "")?" TRUNC(".$field.") = TO_DATE('".$date."', 'YYYY-MM-DD')":" AND TRUNC(".$field.") = TO_DATE('".$date."', 'YYYY-MM-DD')";
                    }
                    ...
                    if($dateStart != null && $dateEnd != null){
                        $whereStatement .= ($whereStatement == "")?" ".$field." BETWEEN TO_DATE('".$dateStart."', 'YYYY-MM-DD') AND TO_DATE('".$dateEnd."', 'YYYY-MM-DD')":" AND ".$field." BETWEEN TO_DATE('".$dateStart."', 'YYYY-MM-DD') AND TO_DATE('".$dateEnd."', 'YYYY-MM-DD')";
                    }else if($dateStart != null){
                        $whereStatement .= ($whereStatement == "")?" TRUNC(".$field.") = TO_DATE('".$dateStart."', 'YYYY-MM-DD')":" AND TRUNC(".$field.") = TO_DATE('".$dateStart."', 'YYYY-MM-DD')";
                    }
                }
            }
        }
        ...
        if($request->input('search.value') != "")
        {
            foreach($filters as $filterKey => $filterValue)
            {
                ...
                if($method == "LIKE"){
                    ...
                    if($date != null){
                        $filterStatement .= ($filterStatement == "")?" TRUNC(".$field.") = TO_DATE('".$date."', 'YYYY-MM-DD')":" AND TRUNC(".$field.") = TO_DATE('".$date."', 'YYYY-MM-DD')";
                    }
                }else if($method == "DATE_BETWEEN"){
                    $dateStart = null;
                    $dateEnd = null;
                    ...
                    if($dateStart != null && $dateEnd != null){
                        $filterStatement .= ($filterStatement == "")?" ".$field." BETWEEN TO_DATE('".$dateStart."', 'YYYY-MM-DD') AND TO_DATE('".$dateEnd."', 'YYYY-MM-DD')":" AND ".$field." BETWEEN TO_DATE('".$dateStart."', 'YYYY-MM-DD') AND TO_DATE('".$dateEnd."', 'YYYY-MM-DD')";
                    }else if($dateStart != null){
                        $filterStatement .= ($filterStatement == "")?" TRUNC(".$field.") = TO_DATE('".$dateStart."', 'YYYY-MM-DD')":" AND TRUNC(".$field.") = TO_DATE('".$dateStart."', 'YYYY-MM-DD')";
                    }
                }
            }
        }
        ...
        if($request->input('start') != "" && $request->input('length') != ""){
            if($this->version == "12"){
                $limit = "OFFSET ".$request->input('start')." ROWS FETCH NEXT ".$request->input('length')." ROWS ONLY";
            }else{
                $limit = "WHERE rowIndex > ".$request->input('start')." AND rowIndex <= ".$request->input('length');
            }
        }
        ...
        if($this->version == "12"){
            ...
            $query = "SELECT T.* FROM ( 
                        ...
                        FROM (
                            ...
                            FROM $table
                                $joinStatement
                            $whereStatement $groupStatement $orderStatement
                        )T)T";
        }
